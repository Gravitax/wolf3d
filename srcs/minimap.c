...
static void		calc_player(t_wolf *data, int sc_x)
{
	...
	while (++i < W_WIDTH)
	{
		...
		SDL_RenderDrawLine(data->renderer, pl.x, pl.y, data->map.depth_buffer[i] * W_WIDTH / (data->map.sc_x * data->map.width) * (cosf(data->raydata.angle)) + pl.x, data->map.depth_buffer[i] * W_HEIGHT / (data->map.sc_x * data->map.height) * (sinf(data->raydata.angle)) + pl.y);
	}
	data->raydata.angle = (data->player.angle - data->player.fov / 2)
			+ ((float)i / (2 * (float)W_WIDTH)) * data->player.fov;
	SDL_SetRenderDrawColor(data->renderer, 155, 150, 150, 100);
	SDL_RenderDrawLine(data->renderer, pl.x, pl.y, data->map.depth_buffer[i / 2] * W_WIDTH / (data->map.sc_x * data->map.width) * (cosf(data->raydata.angle)) + pl.x, data->map.depth_buffer[i / 2] * W_HEIGHT / (data->map.sc_x * data->map.height) * (sinf(data->raydata.angle)) + pl.y);
}
